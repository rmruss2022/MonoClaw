...
+-----------------------------------------------------------------------------------------+
      ...
      +-------------------------------+        |------------------------------------------|
                                               ...
                                               | 4) Build candidate set + eligibility     |
                                               |    gates (state, deadline, activity win)|
                                               | 5) Score each candidate (time-aware)     |
                              ...
                              | - recency_signal  = bucketed by matched feature last_seen_at      |
                              |   (<=7d:1.0, <=30d:0.85, <=90d:0.65, <=180d:0.45, else:0.25)     |
                              | - similarity_base = min(1.0, 0.3 + 0.15*matched_required)         |
                              | - similarity      = 0.8*similarity_base + 0.2*recency_signal      |
                              | - payout_signal   = min(1.0, payout_max_cents/10000)             |
                              | - urgency_signal  = function(days_to_deadline), not boolean       |
                              |   and expired deadlines are hard-filtered out                     |
                             ...
                             | - similarity, similarity_base, recency                              |
                             | - payout, urgency, days_to_deadline, ease                           |
                             | - score                                                             |
                             +-----------------------------------+--------------------------------+
                                                                 |
                                                                 v
...
4. Time-aware behavior: expired deadlines are excluded, urgency uses days-to-deadline, and optional activity windows enforce date-range eligibility.
5. Model retraining does not automatically activate every new candidate; promotion gate controls activation.
