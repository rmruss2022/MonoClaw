...
```typescript
// Read the task spec
const specPath = '{{SPECS_PATH}}/{{TASK_ID}}-spec.md';
...
VALUES ({{PROJECT_ID}}, datetime('now'), 'agent-{{TASK_ID}}', '{{TASK_ID}}', 'Task completed: {{TASK_TITLE}}', 'success');
"
\`\`\`
...
VALUES ({{PROJECT_ID}}, 'agent-{{TASK_ID}}', '{{TASK_ID}}', datetime('now'), 'running');
"
```
...
```bash
sqlite3 {{DATABASE_PATH}} "
SELECT COUNT(*) FROM tasks WHERE project_id={{PROJECT_ID}} AND state != 'done';
"
```
...
1. Read all task specs from `{{SPECS_PATH}}/`
