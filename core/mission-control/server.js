...
// Cache for entire system data (very expensive - multiple API calls)
...
async function getSystemData() {
    ...
    try {
        ...
        // Get cron jobs and parse status (both async)
        ...
        const data = {
            ...
            services: [
                ...
                },
                {
                    ...
                    detail: missionControl ? `Port 18795/cron` : 'Stopped'
                },
                {
                ...
                {
                    name: 'Command Hub API',
                    running: commandHubAPI,
                    detail: commandHubAPI ? `Port 3001 (Backend API)` : 'Stopped',
...
async function getCronJobs() {
    try {
        ...
        // Map to simplified format with status
        return cronData.jobs.map(job => {
            ...
            if (job.schedule.kind === 'cron') {
                ...
                const mins = job.schedule.everyMs / 60000;
                scheduleText = mins < 60 ? `Every ${mins}m` : `Every ${mins/60}h`;
            }
            
            let nextRunText = 'Never run';
            if (lastRunAtMs) {
                const ago = Date.now() - lastRunAtMs;
                if (ago < 60000) nextRunText = 'Just now';
                else if (ago < 3600000) nextRunText = `${Math.floor(ago/60000)}m ago`;
                else if (ago < 86400000) nextRunText = `${Math.floor(ago/3600000)}h ago`;
                else nextRunText = `${Math.floor(ago/86400000)}d ago`;
            }
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.url.startsWith('/data')) {
        ...
        res.end(html);
    } else if (req.url === '/cron' || req.url === '/cron-dashboard.html') {
        const html = fs.readFileSync(path.join(__dirname, 'cron-dashboard.html'), 'utf-8');
        res.writeHead(200, { 'Content-Type': 'text/html' });
        res.end(html);
