      ...
      {dbStats && (
        <div className="mb-6 p-4 bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-500/30 rounded-lg">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold text-purple-300">💾 Database Health</h3>
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
              dbStats.sizeInMB < 10 ? 'bg-green-500/20 text-green-400' :
              dbStats.sizeInMB < 20 ? 'bg-yellow-500/20 text-yellow-400' :
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <div className="text-white font-mono text-lg">{dbStats.totalRecords.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-gray-400 text-xs mb-1">Unique Agents</div>
              <div className="text-white font-mono text-lg">{dbStats.uniqueAgents}</div>
            </div>
            <div>
              <div className="text-gray-400 text-xs mb-1">Data Age</div>
              <div className="text-white font-mono text-lg">{dbStats.ageInDays}d</div>
            </div>
            <div>
              <div className="text-gray-400 text-xs mb-1">Cleanup Status</div>
              <div className="text-white font-mono text-lg">
                {dbStats.totalRecords < 1000 ? '✅' : dbStats.totalRecords < 5000 ? '⚠️' : '🚨'}
              </div>
            </div>
          </div>
          {dbStats.sizeInMB > 15 && (
            <div className="mt-3 p-2 bg-red-500/10 border border-red-500/30 rounded text-xs text-red-300">
              ⚠️ Database is large! Consider running cleanup more frequently.
            </div>
          )}
        </div>
      )}
